            ...
            <div className="flex-1">
              ...
              <div className="space-y-4">
                {components.map((component) => (
                  <div key={component.id} className="border rounded-lg p-4 flex flex-row">
                    <img src={component.imageUrl} alt={component.name} className="w-48 h-48 object-cover mr-4" />
                    <div className="flex flex-col flex-1">
                      <h3 className="text-lg font-semibold mb-2">{component.name}</h3>
                      <p className="text-gray-600 mb-2">{Object.entries(component.specs).map(([key, value]) => `${key}: ${value}`).join(', ')}</p>
                      <p className="text-lg font-bold mb-4">${component.price.toFixed(2)}</p>
                      {component.name === "Intel Core i9-13900K" ? (
